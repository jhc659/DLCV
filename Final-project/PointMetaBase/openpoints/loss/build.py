...
class FocalLoss(torch.nn.Module):
    ...
    def forward(self, logit, target):
        ...
        pesudo_target = target.clone()
        pesudo_target[pesudo_target == -100] = 0

        # print(f"logit : {logit.type()} target : {target.type()} alpha : {self.alpha.type()}")
        logpt = F.log_softmax(logit) # N*H*W,C
        logpt = logpt.gather(1, pesudo_target) # N*H*W,1
        ...
        if self.alpha is not None:
            ...
            at = self.alpha.gather(0, pesudo_target.data.view(-1))
            logpt = (logpt * at).masked_fill_(target.data.view(-1) == -100, 0) #ignore -100
    
        loss = -1 * (1 - pt) ** self.gamma * logpt
        if self.size_average:
            return loss.sum() / (target.data.view(-1) != -100).int().sum()
        else:
            return loss.sum()
