...
def render_viewpoints(
    ...
    if savedir is not None and dump_images:
        for i in trange(len(rgbs)):
            ...
            fn = os.path.split(filenames[i])[-1]
            # print("file_path: ", fn)
            filename = os.path.join(savedir, '{}.png'.format(fn))
...
def load_everything(args, cfg):
    ...
    if args.json_path:
        ...
        data_dict = load_data(cfg.data, json_path)
    else:
...
if __name__ == '__main__':
    ...
    if args.render_test or args.render_train or args.render_video:
        ...
        model = utils.load_inference_model(model_class, ckpt_path).to(device)
    ...
    if args.render_test:
        ...
        if args.json_path == '':
            rgbs, depths, bgmaps = render_viewpoints(
                ...
                gt_imgs=[
                    data_dict['images'][i].cpu().numpy() for i in data_dict['i_test']
                ],
                savedir=testsavedir,
